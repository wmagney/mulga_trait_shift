...
nBoot <- 1000
lrStat <- rep(NA, nBoot)
SLA.anova.null <- aov(log(sla) ~  rainfall, data = trait_SLA) #null model
SLA.anova.fit <- lmer(log(sla) ~ rainfall + (1|landform) , data = trait_SLA, REML = F) # full model
lrObs <- 2 * logLik(SLA.anova.fit) - 2 * logLik(SLA.anova.null) # observed test stat
for (iBoot in 1:nBoot)
{
  trait_SLA$logSLASim <- unlist(simulate(SLA.anova.null)) # resampled data
  bNull <- aov(logSLASim ~  rainfall, data = trait_SLA) # null model
  bAlt <- lmer(logSLASim ~ rainfall + (1 | landform), data = trait_SLA, REML = F) # full model
...
```{r}
# Extract the residuals from the model
residuals <- residuals(height.GLMM.gamma)
...
plot_grid(basal.vs.heigh_plots, basalvscanopy, canopyvsheight, b.a_graph, heightvsrainfall, nrows = 2, ncol = 2, rel_widths = c(1,1))
...
ggsave("stand_comp.png", width = 10, height = 10)
...
ggsave("dieoff_comp.png", width = 10, height = 10)
```
