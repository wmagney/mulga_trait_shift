...
```{r}
spatial_plot_D2h$lagged_density <- lag(plot_D2h$log.sub.dens, 1)
```

```{r}
final.density.lag.fit <- aov(log.sub.dens ~ rainfall + lagged_density, data = spatial_plot_D2h)
...
spatial_plot_D2h$Resid <- residuals(final.density.aov)


bubble(spatial_plot_D2h, "Resid")

```


```{r}
coords = cbind(spatial_plot_D2h$xcoord, spatial_plot_D2h$ycoords)
w = fields:::rdist(coords)
Moran.I(x = spatial_plot_D2h$log.sub.dens, w = w)
```
