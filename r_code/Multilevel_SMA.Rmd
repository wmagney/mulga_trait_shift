...
## test fit across levels of subplots 



```{r}
sma.sub.fit.test <- sma(height ~ basal_area * subplot,  log="xy", data=tree_structure)
sma.sub.fit.test 


summary(sma.sub.fit.test)

```


```{r}
sma.int.fit<- with(tree_structure, elev.com(log(height), log10(basal_area), subplot))
...
```{r}
sma.test.fit <- tree_structure%>%
  ggplot(aes(basal_area, height, colour = rainfall, shape = plots))+
